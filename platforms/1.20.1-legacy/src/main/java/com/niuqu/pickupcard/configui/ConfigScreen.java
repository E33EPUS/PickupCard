...
public final class ConfigScreen extends Screen {
  ...
  private final CardPreview preview = new CardPreview(new ItemStack(Items.DIAMOND));
  ...
  public void showSection(int index) {
    this.section = Mth.clamp(index, 0, SECTIONS.length - 1);
  ...
  private void rebuild() {
    ...
    int controlWidth = Mth.clamp((this.contentRight() - this.contentLeft()) / 2, 96, 132);
    ...
    for (ConfigOption optionx : this.options) {
      this.widgets.add((ConfigWidget)(switch (optionx.kind()) {
        ...
        case STRING, LIST -> new TextWidget(optionx, this.font, controlX, 0, controlWidth, 18);
      }));
    }
  ...
  private void layoutRows() {
    for (int i = 0; i < this.widgets.size(); i++) {
      this.widgets.get(i).setY(this.listTop() + i * 23 - (int)this.scroll);
    }
  }

  public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    graphics.fill(0, 0, this.width, this.height, -267778542);
  ...
  private void drawHeader(GuiGraphics graphics) {
    graphics.drawString(this.font, this.title, 10, 8, -659457, true);
    graphics.drawString(this.font, Component.translatable("pickupcard.config.hint"), 10, 20, -6516811, false);
    int x = this.width - 170 - 10;
    panel(graphics, x, 4, x + 170, 44, 4.0F, -535162842, -10389);
    graphics.drawString(this.font, Component.translatable("pickupcard.config.preview"), x + 6, 8, -10389, false);
    this.preview.render(graphics, x + 2, 16, 166, 22);
  }

  private void drawSidebar(GuiGraphics graphics, int mouseX, int mouseY) {
    panel(graphics, 6, 46, 106, this.height - 26 - 2, 4.0F, -534637008, 956290923);
    ...
    for (int i = 0; i < SECTIONS.length; i++) {
      ...
      if (active) {
        graphics.fill(9, y + 3, 11, y + 20 - 3, -10389);
      }

      graphics.drawString(this.font, Component.translatable("pickupcard.config.section." + SECTIONS[i]), 17, y + 6 + 1, active ? -659457 : -6516811, true);
    }
  }
  ...
  private void drawContent(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ...
    try {
      for (int i = 0; i < this.widgets.size(); i++) {
        ...
        if (widget.getY() + widget.getHeight() >= top && widget.getY() <= bottom) {
          widget.render(graphics, mouseX, mouseY, partialTick);
          String label = this.font.plainSubstrByWidth(this.options.get(i).label().getString(), Math.max(24, widget.getX() - left - 14));
          graphics.drawString(this.font, label, left + 4, widget.getY() + (widget.getHeight() - 8) / 2 + 1, -659457, true);
    ...
    if (this.maxScroll > 0.0) {
      ...
      graphics.fill(right + 2, top, right + 4, bottom, 1711276032);
      graphics.fill(right + 2, barY, right + 4, barY + barHeight, -10389);
    }
  }
  ...
  private void drawFooter(GuiGraphics graphics, int mouseX, int mouseY) {
    ...
    graphics.drawString(this.font, label, x + (w - this.font.width(label)) / 2, y + 3, -659457, true);
  }
    ...
    if (this.hovered != null) {
      ...
      if (!lines.isEmpty()) {
        ...
        for (FormattedCharSequence line : lines) {
          boxWidth = Math.max(boxWidth, this.font.width(line));
        }

        boxWidth += 14;
        int boxHeight = lines.size() * lineHeight + 8;
        int x = Mth.clamp(mouseX + 10, 4, Math.max(4, this.width - boxWidth - 4));
        int y = Mth.clamp(mouseY + 10, 4, Math.max(4, this.height - boxHeight - 4));
        panel(graphics, x, y, x + boxWidth, y + boxHeight, 4.0F, -266858460, -10389);

        for (int i = 0; i < lines.size(); i++) {
          graphics.drawString(this.font, lines.get(i), x + 7, y + 4 + i * lineHeight, -659457, false);
        }
      }
    }
  }
    ...
    if (button != 0) {
      ...
      if (mouseX >= bx && mouseX < bx + w && mouseY >= by && mouseY < by + 14) {
        ...
        for (ConfigWidget widget : this.widgets) {
          if (widget.isMouseOver(mouseX, mouseY) && mouseY >= this.listTop() && mouseY < this.listBottom()) {
            target = widget;
            break;
          }
        }
        ...
        for (ConfigWidget widgetx : this.widgets) {
          widgetx.setFocused(widgetx == target);
        }
  ...
  public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
    if (this.maxScroll > 0.0 && mouseX >= this.contentLeft() - 4) {
      this.scroll = Mth.clamp(this.scroll - delta * 16.0, 0.0, this.maxScroll);
      this.layoutRows();
      return true;
    } else {
  ...
  public boolean charTyped(char codePoint, int modifiers) {
    for (ConfigWidget widget : this.widgets) {
      if (widget.isFocused() && widget.charTyped(codePoint, modifiers)) {
        return true;
      }
    }
