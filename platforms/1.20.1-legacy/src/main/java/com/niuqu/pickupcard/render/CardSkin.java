...
public enum CardSkin {
  ...
  void draw(GuiGraphics graphics, int x, int y, int width, int height, double alpha) {
    ...
    if (!(a <= 0.0F)) {
      Minecraft.getInstance().getTextureManager().getTexture(this.texture).setFilter(true, false);
      ...
      try {
        graphics.blit(this.texture, x - head, dstY, capW, dstH, 0.0F, 0.0F, capW * 4, 184, 640, 184);
        if (midW > 0) {
          graphics.blit(this.texture, x + edge, dstY, midW, dstH, capW * 4, 0.0F, 448, 184, 640, 184);
        }

        graphics.blit(this.texture, x + width - edge, dstY, capW, dstH, 640 - capW * 4, 0.0F, capW * 4, 184, 640, 184);
      } finally {
