...
public final class ConfigOption {
  ...
  public Component label() {
    return Component.translatable("pickupcard.config." + this.key);
  }

  public List<FormattedCharSequence> tooltip(int wrapWidth) {
    String tipKey = "pickupcard.config." + this.key + ".tooltip";
    return !I18n.exists(tipKey) ? List.of() : Minecraft.getInstance().font.split(Component.translatable(tipKey), wrapWidth);
  }
  ...
  public void setText(String text) {
    if (this.kind == ConfigOption.Kind.STRING) {
      ...
      // 【反编译修复 · 2026-09-17】原文是 `this.value.set(parts)`，但反编译把字段类型变成了
      // `ConfigValue<?>`（通配符），javac 报 CAP#1 不兼容。原版源码里这个字段是带类型的
      // （list 就是 ConfigValue<List<? extends String>>），类型信息在字节码里被擦掉了。
      // 这里用 raw 强转绕开 —— 行为与原文一致，只是把丢失的类型信息补一次。
      ((ConfigValue) this.value).set(parts);
    }
  }
  ...
  private void set(Object raw) {
    // 【反编译修复】同上：通配符字段收 Object 要 raw 强转。
    ((ConfigValue) this.value).set(raw);
  }
