...
| 阴影 | **按轮廓**逐元素投（竖条 + 图标格 + 名字框三个影子），不再是包住整卡的大矩形；软边是 NanoVG 的 `nvgBoxGradient`（**不是**高斯模糊）+ 竖条影子跟着竖条的头走（原来垂直居中，竖条长到一半时影子比它还高）。有 `design/shadow_probe.py` 盯着 |
...
2026-09-17 收口成 NanoVG 时，影子因为"把卡内局部坐标当屏幕坐标用"整整齐齐画到了屏幕
左上角：测量页全绿，是**棋盘底的 harness 截图**一眼看出来的。所以现在有了
`design/shadow_probe.py`（判据：卡左缘外必须比背景暗 ≥ 12、最外侧要回到背景 ±6、
竖条顶再往上 20px 必须是背景），并带 `--selftest` 负例对照；输入就是棋盘底的
`pickupcard-harness-p*`。**"这条链看不见的东西"要单独给它一条链，别指望主门禁。**

剩下的**已知未量**：软边是 NanoVG 的 `nvgBoxGradient`（框内实心、往外 feather 像素渐变到
透明），**不是** CSS 的高斯模糊 —— 草稿那边是 `filter: drop-shadow`，两者的衰减形状不同，
谁对还没定。要定就得浅灰底 + 沿一条竖线扫灰度剖面，草稿与游戏并排比。**先做尺子再调。**
