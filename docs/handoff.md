...
| ② | 配置界面重铸（三个列表 / 等宽 / 剩余空间做预览 / 带动画 / 排版） | **四步全做完**（NvgUi 能力 → 三列骨架 → 单列行 → 预览样例+短动画+排版）。**只剩视觉复核**，见"下一步 0" |
...
都已修并真机验证；②（配置界面重铸）四步全做完（①NvgUi 能力 → ②三列骨架 → ③单列行 →
④预览样例 + 短动画 + 排版），**只剩"看截图"这一道视觉复核没做完**；⑤（主题数据化）用户只
挑了三项做，主题数据化当时明确不做。**

0. **【下个会话第一件事】视觉复核新截图**。本轮视觉额度用尽（`VISION_RATE_LIMITED`），
   所以新截图只看了四张、没看全。先 `cp` 成 `.png` 再喂看图工具（截图**没有扩展名**）：
   - `platforms/1.20.1-forge/run/screenshots/pickupcard-config*` = **427×240 的最新一套**
     （22:39 那次自动跑测，含 `14b7d4e` 的标签列修复，日志 `画了=14/14`）。
   - `run/screenshots/before-tab-fix/` = 修复**之前**同一套（标签列空着）—— 留着当"修好了没"的对照。
   - 320×180 那一套（预览收起那档）被 `rm` 掉了，要的话 `-PharnessGuiScale=4` 再跑一次（约 2 分钟）。
   **要专门看的三件事**：① 四条标签在不在（`14b7d4e` 之前整列没画）；
   ② 行悬停高亮带 + 标签变亮（`pickupcard-config-hover`，读数 `0.00 → 0.75 → 1.00` 已证曲线跑过，
   没证"画出来好不好看"）；③ 换样例淡入中段，图标有没有跟着半透明
   （`pickupcard-config-preview-fade`；图标走的是 `RenderSystem.setShaderColor`，只有这张图能证）。
1. **真机验收尾巴**：超长名字截断（Q8 的屏宽上限）、多分辨率（guiScale 2/4/5）。
   **`appearMode = CLIP` 已验**（2026-09-17，dev 跑测目录把 TOML 改成 CLIP 跑 `-PharnessAuto=shot`）：
   ...
   实际窗口从左边长起 → **图标端先出**。
2. **待决的一个数**：`LayoutSettings.MIN_AUTO_PERCENT = 60`（自动缩放的下限）。0.6 倍下 MC 字体
   只剩约 5.4px，**可能太小读不出来** —— 要真机看 320×180 那一档再定（`-PharnessGuiScale=4`）。
