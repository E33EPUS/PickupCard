...
| 项目 | 参考图 | 我们 |
| --- | --- | --- |
| **对齐** | **左缘固定**（竖条左缘极差 0） | ✅ **已改成左**（`LayoutSettings` 默认 + TOML 默认 + `measure.html`） |
| **竖条宽 / 卡高** | 0.1667（16/96） | 0.1562（15/96）—— **决定保持 5px**，见 `plan-ui.md` |
| 竖条高 / 上下内缩 / 两处间距 / 图标格 / 级差 | 0.875 / 0.0625 / 0.125 / 1.000 / 1.1875 | **全部一致** |
| 框内顶部高光 | 没有 | ✅ **已补到实现里**（`TrioCardPainter.topHighlight()`，1px） |
| 经验卡图标 | 有图标（下界之星） | ✅ **已补**：固定渲染 `Items.NETHER_STAR` |

**对齐这条是配置能到的**（用户拍板后已把它变成默认值）：`stickTo=LEFT` + `leftEdge=16` 实测得到
...
### 2c. 设计稿 vs 实现：还差这几处（客观门之外）

| 差异 | 现状 |
| --- | --- |
| ~~顶部高光线~~ | ✅ 已做（用户选"补到实现里"）。参考图没有这条线，设计稿有，用户要设计稿那版 |
| ~~经验卡的图标~~ | ✅ 已做：改渲染 `Items.NETHER_STAR`。实测图标区亮度图是四角星、核心 `#FFFFD2` |
...
1. 形状层那轮：**修 `warmUp()` 的真根因**，顺手把**竖条比逻辑尺寸窄 0.3~0.5 设备像素**
   一起查掉（同一个批处理/着色器路径上的事）
2. 退场动画 / 合并跳动 / `appearMode = CLIP` 真机对照 / 超长名字验收
3. 强提醒：**`config/` 在 `run/` 里，被 gitignore。** 改了 `PickupCardConfig` 的默认值之后，
   老机器上那份 TOML **不会**跟着变 —— 要么删掉让它重新生成，要么手动改。
   这一轮第一次改完默认对齐没生效就是栽在这上面（代码默认已是 LEFT，文件里还写着 RIGHT）。
...
- **过滤默认什么都不丢**（2026-09-17 定案）：以前那张"内置忽略表"会把泥土/圆石/沙子一类
  静默丢掉，玩家的观感就是"mod 坏了"（实测踩到）。现在默认每次拾取都弹卡，想安静自己写黑名单。
  **排查"某件东西没弹卡"时，先看日志里有没有"被过滤器丢弃"**，那是唯一会丢卡的地方。
- **改了代码默认值，别忘了跑测目录里那份配置。** `run/config/*.toml` 被 gitignore，
  但它**优先于代码默认值**。这一轮改完默认对齐去跑测，结果一点没变 —— 因为文件里还写着旧值。
  删掉让它重新生成才对。**"改了没反应"十有八九是这种。**
