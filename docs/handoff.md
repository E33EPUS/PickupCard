...
| | 现状 |
| --- | --- |
| 底部留白 | **不再是魔数**：`shared/layout/HudSafeZone` 按原版 HUD 的矩形算出来（数字与出处都写在那个类里）—— 让到**手持物品名那一行**（`Gui#renderSelectedItemName`：`y = H-59`、居中、宽度随名字变化）之上，即 **62 逻辑px**。口径是"让开它 = 让开下面所有行"（护甲 H-49、血量 H-39、经验 H-29、快捷栏 H-22）。**用户报过三次重叠**：前两次都在改魔数（16 → 52），第二次漏的就是这行名字 |
