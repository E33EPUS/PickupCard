...
| 入场 | 卡片**原地出现**；总时长 **800ms**：竖条**从上头往下**长（前 30% = 240ms；不是从中间往两头 —— 用户真机报的）、内容从**隧道口**横向滑出（18% = 144ms 起跑，656ms 走完）。曲线：竖条 `CubicBezier.BAR`、内容 `CubicBezier.CONTENT`（**故意的偏离草稿** —— 原来那条 t=0.25 就走 0.757，用户报「冲得太快」；换 Material 标准曲线后只走 0.237，草稿已同步改）。**隧道口 = 竖条右缘 + 间隙**，三条绘制路径共用 `RevealWindow`；**内容裁、竖条不裁**（与草稿的 `drop-shadow` 同一套）；**影子浓度跟着形状出现多少走**（用户报「影子一出来就是满的」）|
...
- **"曲线/裁剪在别处做"是一条只写在注释里的约定，等于没有。** 2026-09-17 玩家报"卡片
  ...
  进度的定义处），窗口收成 `shared` 的 `RevealWindow` 一个类，三条绘制路径（SDF 裁剪 /
  NanoVG 裁剪 / 原版内容裁剪）都用它。**同一个几何算两遍，就一定会有一处漏。**
