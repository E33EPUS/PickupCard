...
## 阴影：已定位现象，根因未解（2026-09-17）

**现象**：阴影 quad 的**可见部分**渲染成不透明纯色（alpha=1），而不是设定的半透明。
被卡片盖住的部分看不见，所以表现是"卡片下方露出一条 5 逻辑像素高的纯黑带"。

**证据**（guiScale 2，`Default` 主题）：

| 变体 | 纯黑像素 |
| --- | --- |
| 原样（alpha=90, blur=6，SDF 软化） | 16791 |
| `shadowBlur = 0`（SDF 软化宽度 0） | **14** |
| `shadowAlpha = 0`（完全不画阴影） | **14** |
| 多层硬边叠加（假模糊，5 层） | 24411 |
| 单张硬边矩形 | 12930 |

- 纵切面（x=700）显示：卡身正常 → 红色辅助线 → **y 144–153 共 10 行 RGB(0,0,0)**，
  正好是阴影露在卡下方的那一段。
- 横切面（y=110）只在 x=634–637 / 702–709 / 822–825 是黑 —— 即阴影左右两侧露出的边。
- **染成品红**后那块是 `RGB(255,0,255)`，即 alpha 完全饱和 —— 顶点上传的 alpha 没进混合。
- 但同一批里卡片的玻璃填充（alpha=209）看不出异常，所以**不能简单说"这批的 alpha 全丢了"**。

**当前处置**：`default.json` 的 `shadowAlpha` 置 0，先不用阴影。
`ShapeBatch.shadow()` 与着色器的 `u_soft` 保留（它们是形状层的通用能力），但已知
`soft > 0` 会触发上面这个问题，暂不使用。

**复现步骤**：把 `styles/default.json` 的 `shadowAlpha` 改回 90 → 跑
`./gradlew runClient -PharnessAuto=shot` → 量 `run/screenshots/pickupcard-harness-p1`
的纯黑像素（正常应 <20，出问题会 >10000）。

**下一步查法**（还没做）：
1. 用 `gui.fill(x, y, w, h, 0x5A000000)` 画同一个矩形 —— 原版路径。若正常，
   问题锁定在形状层；若也黑，问题在渲染阶段/帧缓冲。
2. 打印 `ShapeBatch.stats().flushes()`，确认这个 quad 没有被重复提交。
3. 核 `RenderType.end()` 之后 `setupRenderState/clearRenderState` 是否配对，
   以及半透明混合状态在多次 `endBatch` 之间是否被正确重置。
