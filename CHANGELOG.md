...
> **注意：这一段的 jar 被就地替换过三次（2026-09-19 深夜）。** 用户要求不改版本号：
> 第一次是五处验收修复，第二次是配置界面架构重构（见"架构重构"小节），第三次是
> 与 UI Deck 同装的 JPMS 打包修复（见下）。旧产物 `sha256 c9952758…` → `9ef6a724…`
> → `6d196671…`，现行产物以 `pickupcard-Forge-1.20.1-0.2.2.jar` 当时的 sha256 为准
> （本次 `518896c9…`）。判据：包里**有** `ConfigPageSpec` 类 = 重构后；
> **有** `META-INF/jarjar/` = 打包修复后。
