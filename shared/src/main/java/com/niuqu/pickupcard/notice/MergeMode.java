...
/**
 * 「什么算同一个物品」—— 合并的粒度。四档，默认 {@link #SAME_NBT}（同名同 NBT）。
 * <p>
 ...
 * <b>哪些拾取算同一件东西</b> —— 那是个谓词，不是时间。
 * <p>
 * 【想法来自哪里，代码不是】"合并粒度是谓词、不是时长"这个思路是从同赛道最成熟的那版
 * Loot Journal（**ARR，全权保留**）读出来的，记在 `docs/reference-loot-journal.md`。
 * <b>命名与实现全是我们自己的</b>：它的枚举字面量一个都没用、它的代码一行都没进来。
 * <p>
...
public enum MergeMode {
    ...
    /** 这一档要不要把 NBT 编进身份键里（只有 SAME_NBT 要）。 */
    public boolean usesNbt() {
        return this == SAME_NBT;
    }

    /** 这一档要不要比外观档位（只有 SAME_NBT 要；理由见 {@code ItemIdentity#lookOf}）。 */
     ...
     * 这一次拾取要不要单独一个身份。
     *
     * @param customNamed 这件物品改过名字吗（{@link #SAME_ITEM_KEEP_NAMED} 下改过名的要自己一张卡）
     */
    public boolean uniquePerPickup(boolean customNamed) {
        return this == NEVER || (this == SAME_ITEM_KEEP_NAMED && customNamed);
    }
}
