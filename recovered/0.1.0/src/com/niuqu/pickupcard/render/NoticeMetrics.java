...
public record NoticeMetrics(
  ...
  public static NoticeMetrics measure(NoticeStyle style, Font font, Notice notice, NoticeRenderSettings settings) {
    ...
    boolean durability = card && settings.showDurability() && notice.stack().m_41768_();
    int width = padX * 2 + iconSize + iconGap + font.m_92895_(countText) + badgePad * 2;
    if (card && settings.showName()) {
      width += font.m_92895_(NoticeText.displayName(notice, settings, font)) + iconGap;
    }

    if (newBadge) {
      width += font.m_92895_("NEW") + badgePad * 2 + iconGap;
    }
