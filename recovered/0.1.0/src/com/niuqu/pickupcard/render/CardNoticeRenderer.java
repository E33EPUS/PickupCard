...
public final class CardNoticeRenderer {
  ...
  static void render(GuiGraphics graphics, Font font, Notice notice, NoticeMetrics m, double alpha, NoticeRenderSettings settings, long now) {
    ...
    CardSkin skin = settings.raritySkins() ? CardSkin.of(stack.m_41791_()) : CardSkin.COMMON;
    ...
    if (sdf) {
      SdfRectRenderer.Sheen sweep = sheen(notice, m, settings, now, stack.m_41793_(), skin);
    ...
    int newWidth = 0;
    if (m.newBadge()) {
      newWidth = font.m_92895_("NEW") + m.badgePad() * 2;
      if (sdf) {
        ...
        graphics.m_280509_(cursor, newY, cursor + newWidth, newY + newHeight, RenderColors.withAlpha(settings.accentColorArgb(), alpha));
      }

      cursor += newWidth + m.iconGap();
    }

    int countX = m.width() - m.padX() - font.m_92895_(count);
    if (m.durability()) {
    ...
    if (m.newBadge()) {
      graphics.m_280056_(
        font, "NEW", newX + (newWidth - font.m_92895_("NEW")) / 2, newY + (newHeight - 9) / 2 + 1, RenderColors.withTextAlpha(-15069146, alpha), false
      );
    }

    graphics.m_280056_(font, name, cursor, textY, nameArgb, true);
    graphics.m_280056_(font, count, countX, textY, accentArgb, true);
  }
  ...
  private static void drawDurability(GuiGraphics graphics, ItemStack stack, NoticeMetrics m, double alpha) {
    ...
    int barY = m.height() - 3;
    double left = 1.0 - (double)stack.m_41773_() / Math.max(1, stack.m_41776_());
    double clamped = Math.max(0.0, Math.min(1.0, left));
    int filled = (int)Math.round(barWidth * clamped);
    graphics.m_280509_(barX, barY, barX + barWidth, barY + 2, RenderColors.withAlpha(-15458278, alpha));
    graphics.m_280509_(barX, barY, barX + filled, barY + 2, RenderColors.withAlpha(durabilityColor(clamped), alpha));
  }
